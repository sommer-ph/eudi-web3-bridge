...
echo
echo "------------------------------------------------------"
echo " zk-Proof CLI – Blockchain Wallet Key Derivation"
echo "------------------------------------------------------"
echo
...
# Paths and constants
###############################################################################
CIRCUIT_NAME="blockchain-wallet-key-derivation"
BUILD_DIR="build"
INPUT_DIR="input/cred-bind"
DEST_FILE="${INPUT_DIR}/test-blockchain-wallet-key-derivation.json"
...
time_step() {
  ...
  perf_log+=("$(printf '{"step":"%s","seconds":%s}' "$label" "$duration_sec")")
}
...
time_step "compile_circuit" circom "circuits/cred-bind/c4/${CIRCUIT_NAME}.circom" \
  --r1cs --wasm --sym \
