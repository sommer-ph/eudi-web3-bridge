...
function toLimbs43(decStr) {
  ...
  return limbs.reverse(); // Länge 6
}

// ── util: SHA-256(messageStr) → BigInt (little-endian) ────────────
function sha256BigIntBE(asciiStr) {
  const h = crypto.createHash("sha256").update(asciiStr, "ascii").digest();
  let acc = 0n;
  for (let i = 0; i < 32; i++) acc = (acc << 8n) + BigInt(h[i]); // BE
  return acc.toString();
}
...
(function main() {
  ...
  const hashLimbs = toLimbs43(sha256BigIntBE(messageStr));
