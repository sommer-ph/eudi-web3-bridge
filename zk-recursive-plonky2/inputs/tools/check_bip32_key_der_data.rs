...
fn main() -> Result<()> {
    let json: Input = serde_json::from_slice(
        &fs::read(PathBuf::from("inputs/experiments/bip32_key_der.json"))?
    )?;

    println!("=== CHECKING BIP32 KEY DERIVATION INPUT DATA ===");

    // ---------- 1. Parse Parent Public Key ----------
    println!("\n1. Parsing parent secp256k1 public key...");
    ...
    let pk_0 = K256Public::from_sec1_bytes(&pk_0_bytes)?;
    println!("Parent public key parsed successfully");

    // ---------- 2. Check BIP32 Non-Hardened Derivation ----------
    println!("\n2. Verifying BIP32 non-hardened key derivation...");
    ...
    let parent_pubkey_compressed = pk_0.to_encoded_point(true);
    ...
    let parent_projective = pk_0.to_projective();
    ...
    println!("\nInput is VALID for BIP32 Key Derivation Proof Generation!");
