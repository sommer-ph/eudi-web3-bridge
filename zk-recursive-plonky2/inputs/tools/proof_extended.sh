...
###############################################################################
INPUT_DIR="inputs"
SRC_FILE="../zk-backend/data/proof-preparation/${USER_ID}_recursive_extended.json"
DEST_FILE="${INPUT_DIR}/input_extended.json"
...
if [[ "$DERIVE_MODE" == "poseidon" ]]; then
    ...
    echo "Building generate_poseidon_extended tool ..."
    cargo build --release --bin generate_poseidon_extended

    echo "Converting input for Poseidon derivation ..."
    cargo run --release --bin generate_poseidon_extended -- --input "$DEST_FILE" --output "$DEST_FILE"
