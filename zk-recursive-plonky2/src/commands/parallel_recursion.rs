...
pub fn build_parallel_circuits(signature_mode: SignatureMode, derivation_mode: DerivationMode) -> ParallelCircuits {
    ...
    // Build circuits independently (no dependencies between c1, c2, c3, c4)
    ...
    println!("Building C2 circuit (Public Key Decode and Check)...");
    let c2_start = Instant::now();
    let c2 = build_c2_circuit();
    println!("C2 circuit built in {:?} ({} gates)", c2_start.elapsed(), c2.data.common.degree());
    
    println!("Building C3 circuit (Hash Calculation + Signature Verification)...");
    ...
    println!("Building C5 circuit (BIP32 Key Derivation + recursive verification of c1, c2, c3, c4) with {:?} derivation mode...", derivation_mode);
    let c5_start = Instant::now();
    let c5 = build_c5_circuit_optimized(&c1.data.common, &c2.data.common, &c3.data.common, &c4.data.common, derivation_mode.clone());
    ...
    ParallelCircuits { c1, c2, c3, c4, c5 }
}
...
pub fn generate_parallel_recursive_proof(
    ...
    // === PARALLEL STEPS: Generate C1, C2, C3, C4 Proofs ===
    println!("\n=== PARALLEL STEP: C1 PROOF (EUDI Key Derivation) ===");
    let c1_proof = generate_c1_proof(&circuits.c1, &input, build_dir)?;

    println!("\n=== PARALLEL STEP: C2 PROOF (Public Key Decode and Check) ===");
    let c2_proof = generate_c2_proof(&circuits.c2, &input, build_dir)?;

    println!("\n=== PARALLEL STEP: C3 PROOF (Hash Calculation + Signature Verification) ===");
    ...
    println!("\n=== FINAL STEP: C5 PROOF (BIP32 Key Derivation + Recursive Verification of c1, c2, c3, c4) ===");
    let c5_proof = generate_c5_proof(&circuits.c5, &circuits.c1, &circuits.c2, &circuits.c3, &circuits.c4, &input, &c1_proof, &c2_proof, &c3_proof, &c4_proof, build_dir)?;
...
/// Generate C2 proof (Public Key Decode and Check)
...
/// Generate C3 proof (Hash Calculation + Signature Verification)
...
/// Generate C5 proof (BIP32 Key Derivation + Recursive Verification of c1, c2, c3, c4)
