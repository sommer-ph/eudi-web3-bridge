...
/// Targets for the C5 circuit (Key Derivation + recursive verification of c1, c2, c3, c4).
pub struct C5CircuitTargets {
    // Recursive verification targets for all 4 circuits
    pub c1_proof: plonky2::plonk::proof::ProofWithPublicInputsTarget<D>,
...
/// C5 circuit that implements C5 (BIP32 Key Derivation) + recursive verification of c1, c2, c3, c4.
...
mod tests {
    use super::*;
    use crate::circuits::parallel_recursion::c1::build_c1_circuit;
    use crate::circuits::parallel_recursion::c2::build_c2_circuit;
    ...
    #[test]
    fn test_build_c5_circuit_with_static_parallel_sha512() {
        let c1_circuit = build_c1_circuit();
        let c2_circuit = build_c2_circuit();
        let c3_circuit = build_c3_circuit(SignatureMode::Static);
        let c4_circuit = build_c4_circuit();
        let c5_circuit = build_c5_circuit_optimized(&c1_circuit.data.common, &c2_circuit.data.common, &c3_circuit.data.common, &c4_circuit.data.common, DerivationMode::Sha512);
    ...
    fn test_build_c5_circuit_with_static_parallel_poseidon() {
        ...
        let c3_circuit = build_c3_circuit(SignatureMode::Static);
        let c4_circuit = build_c4_circuit();
        let c5_circuit = build_c5_circuit_optimized(&c1_circuit.data.common, &c2_circuit.data.common, &c3_circuit.data.common, &c4_circuit.data.common, DerivationMode::Poseidon);
